...
public class Main {
     ...
     * 可以通过下面所示的方式，将其设置为监听 UDP 端口 8000 上的数据。
     *
     * nc -l -u -p 8000
     *
     * （这里使用的是 Mac，-l 和 -p 不能同时使用，所以使用 nc -l -u 8000 即可）
     *
     * 运行 LogEventBroadcaster 中的 main 的方法，并指定参数为 8000 /var/log/system.log，用来设置端口和日志文
