...
public class Main {
     ...
     * Finagle 的诞生
     *
     * 为了构建出这个新的架构，我们需要一个高性能的、容错的、协议不可知的、异步的 RPC 框架。在面向服务的架构中，
     * 服务花费了它们大部分的时间来等待来自其他上游的服务的响应。使用异步的库使得服务可以并发地处理请求，并且充
     * 分地利用硬件资源。尽管 Finagle 可以直接建立在 NIO 之 上，但是 Netty 已经解决了许多我们可能会遇到的问
     * 题，并且它提供了一个简洁、清晰的 API。
     *
     * Twitter 构建在几种开源的协议之上，主要是 HTTP、Thrift、Memcached、MySQL 以及 Redis。我们的网络栈
     * 需要具备足够的灵活性，能够和任何的这些协议进行交流，并且具备足够的可扩展性，以便我们可以方便地添加更多的
     * 协议。Netty 并没有绑定任何特定的协议。向它添加协议就像创建适当的 ChannelHandler 一样简单。这种扩展性
     * 也催生了许多社区驱动的协议实现，包 括 SPDY、PostgreSQL、WebSockets、IRC 以及 AWS。
     *
     * Netty 的连接管理以及协议不可知的特性为构建 Finagle 提供了绝佳的基础。但是我们也有一些其他的 Netty 不
